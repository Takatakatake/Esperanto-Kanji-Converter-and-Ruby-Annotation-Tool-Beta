...
from esp_replacement_json_make_module import (
    replace_esperanto_chars,
    convert_to_circumflex,
    measure_text_width_Arial16,
    insert_br_at_half_width,
    insert_br_at_third_width,
    output_format,
    contains_digit,
    import_placeholders,
...
st.title("エスペラント語根の置換アプリ")

# 大まかな使い方の概説を先頭付近にまとめる
st.markdown("""
## はじめに
このアプリは、エスペラント語の語根に対して、漢字や日本語訳などのルビを割り当て、  
その結果を JSON 形式の巨大な“最終置換リスト”としてダウンロードできるものです。

以下の手順で利用してください:
1. 必要な **CSV ファイル**(エスペラント語根→日本語訳 の対応表など) をアップロード、またはデフォルトを使用。
2. 必要に応じて **JSON ファイル**(語根分解ルールやカスタム設定など) をアップロード、またはデフォルトを使用。
...
# 例: 日本語訳ルビリスト
st.markdown("""
**サンプルCSVファイル (日本語訳ルビリスト)**  
エスペラント語根と日本語訳を 1 行ずつ対応づけたCSVです。
この形式に合わせて CSV を作成し、アップロードすることで置換に活用します。
""")
# サンプルファイルのパス
...
st.markdown("---")
st.markdown("""
**サンプルCSVファイル (エスペラント語根漢字対応リスト)**  
こちらはエスペラント語根と漢字（短い単語）を対応づけたCSVの例です。
""")
# サンプルファイルのパス
...
with open(json_file_path2, "rb") as file_json:
    btn_json = st.download_button(
        label="サンプルJSONファイル2ダウンロード(置換後文字列のユーザー設定)",
        data=file_json,
...
- 「アップロードする」を選ぶと、ユーザー自身の CSV を読み込みます。  
- 「デフォルトを使用する」を選ぶと、あらかじめ用意された`日本語訳ルビリスト_20250112_字上符形式.csv`を使用します。
""")
...
if csv_choice == "アップロードする":
    ...
    if uploaded_file is not None:
        ...
        st.warning("CSVファイルがアップロードされていません。アプリを停止します。")
...
st.markdown("""
エスペラント単語の語根分解法やカスタム置換設定を記述した JSON ファイルを読み込みます。  
- 「アップロードする」を選べば、ユーザー自身の設定ファイルが反映されます。  
- 「デフォルトを使用する」を選べば、サンプルの `世界语单词词根分解方法の使用者自定义设置.json` が使われます。
            
**JSONファイルの想定フォーマット**:
- リスト形式で各要素が `[エスペラント語, 優先度または'dflt', オプション指定リスト]` の形になっています。
- 例: `["am", "dflt", ["verbo_s1"]]`
""")
...
if json_choice == "アップロードする":
    ...
    if uploaded_json is not None:
        ...
        st.warning("JSONファイルがアップロードされていません。アプリを停止します。")
        st.stop()
...
st.write("---")
st.write("## JSONファイル2をどうしますか？")

json_choice2 = st.radio("エスペラント単語の語根分解法を追加指定するJSONファイル2をどうしますか？", ("アップロードする", "デフォルト2を使用する"))

json_path_default2 = "./Appの运行に使用する各类文件/替换后文字列(汉字)の使用者自定义设置(基本上完全不推荐).json"

change_dec2 = None  # JSON を格納

if json_choice == "アップロードする":
    uploaded_json = st.file_uploader("JSONファイル2をアップロードしてください", type=['json'])
    if uploaded_json is not None:
        change_dec2 = json.load(uploaded_json)
        st.success("JSONファイル2がアップロードおよび保存されました。")
    else:
        st.warning("JSONファイル2がアップロードされていません。")
        st.stop()
elif json_choice2 == "デフォルト2を使用する":
    # デフォルト JSON を読む
    try:
        with open(json_path_default2, "r", encoding="utf-8") as g:
            change_dec2 = json.load(g)
        st.info("デフォルト2の JSON を使用します。")
    except FileNotFoundError:
        st.error("デフォルト2の JSON ファイルが見つかりません。処理を中断します。")
        st.stop()
...
if st.button("置換リストを作成する"):
    with st.spinner("置換リストを生成中... しばらくお待ちください。"):
        ...
        # ★一番最初だけチェックして、説明用の項目を削除する
        if len(change_dec2) > 0:
            if len(change_dec2[0]) != 4:
                # 最初のリストの要素の数が4つでなければ、これを説明用の項目であると判断して削除する。
                change_dec2.pop(0)

        for i in change_dec2:
            if len(i)==4:
                try:   
